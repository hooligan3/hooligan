...
public interface ProductSql {
	...
	//메인 인기상품받기
	public String mainHitProduct="select rnum,t2.* from(select rownum rnum,t1.* from(select p.* from product p  order by p.present_size desc)t1)t2 where rnum between 1 and 10";
	//브랜드의종류별검색
